...
public final class GridChooser extends Composite {
	...
	public GridChooser(Composite parent) {
		...
		availableFormData.top = new FormAttachment(availableLabel, 5,
				SWT.BOTTOM);
		...
		centerFormData.top = new FormAttachment(_available.getControl(), 0,
				SWT.CENTER);
		centerFormData.left = new FormAttachment(50, 100, -10);

		final FormData selectedLabelFormData = new FormData();
		selectedLabelFormData.top = new FormAttachment(availableLabel, 0,
				SWT.TOP);
		selectedLabelFormData.left = new FormAttachment(_selected.getControl(),
				0, SWT.LEFT);

		final FormData selectedFormData = new FormData();
		selectedFormData.top = new FormAttachment(_available.getControl(), 0,
				SWT.TOP);
		selectedFormData.bottom = new FormAttachment(_available.getControl(),
				0, SWT.BOTTOM);
	...
	private void updateSelection(TableViewer viewer, boolean selected) {
		...
		final List<GridChooserItem> selection = ((StructuredSelection) viewer
				.getSelection()).toList();
		boolean selectionChanged = false;
	...
	private void updateSelection(TableViewer viewer, int movement) {
		...
		boolean selectionChanged = false;
		for (GridChooserItem item : selection) {
			selectionChanged |= item.setSelectionOrder(item.getSelectionOrder()
					+ movement, false);
		}
	...
	private static Rectangle getBounds(TableViewer viewer, GridChooserItem item) {
		for (TableItem tableItem : viewer.getTable().getItems()) {
			if (tableItem.getData() == item) {
				...
				return new Rectangle(parentBounds.x + childBounds.x,
						parentBounds.y + childBounds.y, childBounds.width,
						childBounds.height);
			}
		}
	...
	private static Rectangle getBounds(TableViewer viewer,
	...
	private SelectionListener _changeSelectionListener = new SelectionListener() {
		...
		private void handle(SelectionEvent e) {
			...
			if (_bottom.equals(e.widget)) {
				updateSelection(_selected, getSelectionCount()
						- maxSelectionOrder(_selected) - 1);
				return;
			}
		}
	};
	...
	private static class TableLabelProviderImpl extends LabelProvider implements
	...
	private static class SelectionOrderViewerComparator extends
			ViewerComparator {
		...
		public int compare(Viewer viewer, Object e1, Object e2) {
			if (e1 instanceof GridChooserItem && e2 instanceof GridChooserItem) {
				return _comparator.compare((GridChooserItem) e1,
						(GridChooserItem) e2);
			}
	...
	private static class DragSourceListenerImpl extends TableDragSourceEffect {
		...
		public DragSourceListenerImpl(TableViewer viewer) {
			...
			_viewer = viewer;
			_viewer.addDragSupport(DND.DROP_MOVE,
	...
	private class SelectedDropTargetListener extends TableDropTargetEffect {
		...
		public void drop(DropTargetEvent event) {
			...
			if (transfer.isSupportedType(event.currentDataType)) {
				@SuppressWarnings("unchecked")
				final List<GridChooserItem> selected = new ArrayList<GridChooserItem>(
						((StructuredSelection) transfer.getSelection())
								.toList());
				if (!_selected) {
					boolean selectionChanged = false;
					...
					if (selectionChanged) {
						_viewer.setSelection(new StructuredSelection(selected),
								true);
				...
				int targetIndex;
				if (event.item != null) {
					final GridChooserItem targetItem = (GridChooserItem) ((TableItem) event.item)
							.getData();
				...
				for (GridChooserItem item : selected) {
					if (item.getSelectionOrder() < targetIndex
							&& item.getSelectionOrder() >= 0) {
						inserted = true;
						selectionChanged |= item.setSelectionOrder(targetIndex,
								false);
					} else {
						if (inserted) {
							targetIndex++;
						...
						selectionChanged |= item.setSelectionOrder(
								targetIndex++, false);
					}
				}
