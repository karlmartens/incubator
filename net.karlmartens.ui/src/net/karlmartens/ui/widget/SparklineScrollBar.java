...
public final class SparklineScrollBar extends Composite {
	...
	private IFigure createContents(Listener listener) {
		...
		_thumbFigure.add(_labelFigure,
				new org.eclipse.draw2d.geometry.Rectangle(2, 2, 100, 10));
	...
	public int[] getHighlights() {
		...
		int idx = 0;
		for (int h = _highlight.nextSetBit(0); h >= 0; h = _highlight
				.nextSetBit(h + 1)) {
	...
	public void setSelection(int value) {
		checkWidget();
		final int v = Math
				.min(Math.max(_minimum, value), _maximum - _thumb + 1);
		if (v == _selection)
			return;
	...
	private void refreshHighlights() {
		...
		int previous = 0;
		Point p = null;
		for (int i = _highlight.nextSetBit(0); i >= 0; i = _highlight
				.nextSetBit(i + 1)) {
		...
		for (int i = 0; i < points.length; i++) {
			...
			final org.eclipse.draw2d.geometry.Rectangle newBounds = new org.eclipse.draw2d.geometry.Rectangle(
					x, 0, width, available.height);
	...
	private void refreshThumb() {
		...
		_thumbFigure.setSize(
				Math.max(5, available.width * _thumb / computeDataPoints()),
				available.height);
	...
	private void refreshSparkline() {
		...
		for (int i = 0; i < points.length; i++) {
			...
			final int height = (int) ((double) available.height * _data[i]
					/ max * 0.9);
			...
			final org.eclipse.draw2d.geometry.Rectangle bounds = points[i]
					.getBounds();
			final org.eclipse.draw2d.geometry.Rectangle newBounds = new org.eclipse.draw2d.geometry.Rectangle(
					x, y, width, height);
	...
	private int computeX(int index) {
		...
		final Rectangle available = _track.getClientArea();
		final double idealPointWidth = (double) available.width
				/ (double) computeDataPoints();
	...
	private final class Listener implements DisposeListener, MouseListener,
			MouseTrackListener, org.eclipse.draw2d.MouseListener,
			MouseMotionListener, ControlListener {
		...
		public void mouseDown(MouseEvent e) {
			...
			if (source == _track) {
				final double pct = (double) e.x
						/ (double) _track.getClientArea().width;
				final int points = computeDataPoints();
		...
		public void mouseEnter(MouseEvent e) {
			...
			final Object source = e.getSource();
			if (source == _left) {
				_left.setBackground(display
						.getSystemColor(SWT.COLOR_WIDGET_BACKGROUND));
			}
			if (source == _right) {
				_right.setBackground(display
						.getSystemColor(SWT.COLOR_WIDGET_BACKGROUND));
			}
		}
		...
		public void mouseDragged(org.eclipse.draw2d.MouseEvent e) {
			...
			if (_startLocation != null && e.getSource() == _thumbFigure) {
				...
				final double pct = (double) diff
						/ (double) _track.getClientArea().width;

				final int points = computeDataPoints();
				final int correction = (int) (points * pct);
				setSelection(_selection + correction);
				_startLocation.x = _track.getClientArea().width * _selection
						/ points + _offset;
				return;
			}
		}
	...
	private class Repeater implements Runnable {
		...
		private void schedule() {
			getShell().getDisplay().timerExec(
					Math.max(20, 400 / Math.max(1, _iterations)), this);
		}
	}
}
