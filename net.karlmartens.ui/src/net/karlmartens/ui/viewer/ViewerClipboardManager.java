...
public final class ViewerClipboardManager {
	...
	private void handleCopy() {
		if ((_operations & OPERATION_COPY) == 0)
			return;
		final Rectangle region = computeRegion(_viewer.getControl()
				.getCellSelections());
	...
	private void handlePaste() {
		...
		if (region.width == 1 && region.height == 1) {
			...
			final int width = Math.min(dataRect.width,
					_viewer.doGetColumnCount() - region.x);
			final int height = Math.min(dataRect.height,
					_viewer.doGetItemCount() - region.y);
			targetRect = new Rectangle(region.x, region.y, width, height);
		} else if (dataRect.width == 1 && dataRect.height == 1) {
			// Fill
			final int width = Math.min(region.width, _viewer.doGetColumnCount()
					- region.x);
			final int height = Math.min(region.height, _viewer.doGetItemCount()
					- region.y);
			targetRect = new Rectangle(region.x, region.y, width, height);
	...
	private String[][] readFromClipboard() {
		final Clipboard clipboard = new Clipboard(_viewer.getControl()
				.getDisplay());
	...
	private void doSet(Rectangle region, String[] values) {
		...
		final TimeSeriesEditingSupport timeValueEditingSupport = _viewer
				.getEditingSupport();

		for (int y = region.y; y < (region.y + region.height); y++) {
			final TimeSeriesTableItem item = _viewer.doGetItem(y);
			for (int x = region.x; x < (region.x + region.width); x++) {
				_editingSupportCache._base = getViewerColumn(x)
						.doGetEditingSupport();
				final int index = (y - region.y) * region.width
						+ (x - region.x);
				if (x < firstTimeValueIndex) {
					_editingSupportCache
							.setValue(item.getData(), values[index]);
				} else {
					final int vIndex = x - firstTimeValueIndex;
					try {
						final Double value = Double.valueOf(values[index]);
						timeValueEditingSupport.setValue(item.getData(),
								vIndex,
	...
	private TimeSeriesTableViewerColumn getViewerColumn(int index) {
		final TimeSeriesTableColumn column = (TimeSeriesTableColumn) _viewer
				.doGetColumn(index);
		return (TimeSeriesTableViewerColumn) column.getData(Policy.JFACE
				+ ".columnViewer");
	}
